...
struct ic_decl * ic_parse_decl_type(struct ic_token_list *token_list){
    ...
    if( ! tdecl ){
        puts("ic_parse_decl_type: call to ic_decl_get_tdecl failed");
        free(decl);
        return 0;
    }
    ...
    if( ! ic_decl_type_init(tdecl, ic_token_get_string(token), ic_token_get_string_length(token)) ){
        puts("ic_parse_decl_type: call to ic_decl_type_init failed");
        free(decl);
        return 0;
    }
    ...
    while( (token = ic_token_list_peek_important(token_list)) ){
        ...
        if( ! field ){
            puts("ic_parse_decl_type: call to ic_parse_field failed");

            /* free decl and all contents */
            if( ! ic_decl_destroy(decl, 1) ){
                puts("ic_parse_decl_type: in error tidyup call to ic_decl_destroy failed");
            }
            return 0;
        }
        ...
        if( ! ic_decl_type_add_field(tdecl, field) ){
            puts("ic_parse_decl_type: call to ic_decl_type_add_field failed");

            /* free field and all contents */
            if( ! ic_field_destroy(field, 1) ){
                puts("ic_parse_decl_type: in error tidyup call to ic_field_destroy failed");
            }
    ...
    if( ! token ){
        puts("ic_parse_decl_type: call to ic_token_list_expect for end failed");
        return 0;
    }
     ...
     * successful return
     */
    puts("ic_parse_decl_type: something went wrong in finding end");
...
struct ic_decl * ic_parse_decl_func(struct ic_token_list *token_list){
    ...
    if( ! fdecl ){
        puts("ic_parse_decl_func: call to ic_decl_get_fdecl failed");
        free(decl);
        return 0;
    }
    ...
    if( token->id != IC_IDENTIFIER ){
        printf("ic_parse_decl_func: expected IC_IDENTIFIER token, got: ");
    ...
    if( ! ic_decl_func_init(fdecl, ic_token_get_string(token), ic_token_get_string_length(token)) ){
        puts("ic_parse_decl_func: call to ic_decl_func_init failed");
        free(decl);
        return 0;
    }
    ...
    while( (token = ic_token_list_peek_important(token_list)) ){
        ...
        if( ! arg ){
            puts("ic_parse_decl_func: call to ic_parse_field failed");
            free(decl);
            return 0;
        }
        ...
        if( ! ic_decl_func_add_arg(fdecl, arg) ){
            puts("ic_parse_decl_func: call to if_decl_func_add_arg failed");
            free(decl);
            return 0;
        }
    }
    ...
    if( token->id == IC_ARROW ){
        ...
        if( ! ic_decl_func_set_return( fdecl, ic_token_get_string(token), ic_token_get_string_length(token)) ){
            puts("ic_parse_decl_func: call to ic_decl_func_set_return failed");
            free(decl);
            return 0;
        }
        ...
        if( ! ic_decl_func_set_return(fdecl, "Void", 4) ){
            puts("ic_parse_decl_func: call to ic_decl_func_set_return failed for 'Void'");
            free(decl);
            return 0;
        }
    }
    ...
    while( (token = ic_token_list_peek_important(token_list)) ){
        ...
        if( ! stmt ){
            puts("ic_parse_decl_func: call to ic_parse_stmt failed");
            free(decl);
            return 0;
        }
        ...
        if( ! ic_decl_func_add_stmt(fdecl, stmt) ){
            puts("ic_parse_decl_func: call to ic_decl_func_add_stmt failed");
            free(decl);
            return 0;
        }
    }
    ...
    token = ic_token_list_expect_important(token_list, IC_END);
    if( ! token ){
        puts("ic_parse_decl_func: call to ic_token_list_expect for end failed");
        return 0;
    }
     ...
     * successful return
     */
    puts("ic_parse_decl_func: error occurred in ic_parse_this_is_not_the_end");
    free(decl);
    return 0;
}
